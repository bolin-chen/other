...
def model(x, is_training, scope='class', reuse=None):
  with tf.variable_scope(scope, reuse=reuse):
    ...
    with tf.variable_scope('group1'):
     ...
     output = batch_norm(output, is_training=is_training, eps=1e-3, decay=0.6)
...
def main():
  ...
  y_eval = model(x, is_training=False, reuse=True)
